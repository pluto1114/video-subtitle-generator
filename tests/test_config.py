...
class TestVADConfig:
    ...
    def test_default_vad_config(self):
        ...
        config = VADConfig()
        assert config.voice_enhance_threshold == 0.05
        assert config.vad_min_silence_ms == 10
        assert config.vad_speech_pad_ms == 200
        assert config.vad_min_speech_ms == 5
...
class TestConfig:
    ...
    def test_apply_quality_mode_pro(self):
        ...
        assert config.vad_config.voice_enhance_threshold == 0.08
        assert config.vad_config.vad_min_silence_ms == 30
        assert config.vad_config.vad_speech_pad_ms == 200
        assert config.use_vad is False
    ...
    def test_apply_vad_profile_voice_focus(self):
        ...
        assert config.vad_config.vad_min_silence_ms == 50
    ...
    def test_apply_vad_profile_noise_robust(self):
        ...
        assert config.vad_config.voice_enhance_threshold == 0.4
        assert config.vad_config.vad_min_silence_ms == 200

    def test_apply_vad_profile_sensitive(self):
        """Test applying SENSITIVE VAD profile."""
        config = Config()
        Config.apply_vad_profile(config, VADProfile.SENSITIVE)
    ...
    def test_apply_vad_profile_ultra_sensitive(self):
        """Test applying ULTRA_SENSITIVE VAD profile."""
        config = Config()
        Config.apply_vad_profile(config, VADProfile.ULTRA_SENSITIVE)

        assert config.vad_profile == VADProfile.ULTRA_SENSITIVE
        assert config.vad_config.voice_enhance_threshold == 0.03
        assert config.vad_config.vad_min_speech_ms == 5
        assert config.vad_config.vad_min_silence_ms == 20
        assert config.vad_config.vad_speech_pad_ms == 250
        assert config.use_vad is False
